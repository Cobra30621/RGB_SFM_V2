...
\def\titleZh   {灰階至RGB：卷積神經網路可解釋性模型的優化與擴展}   % Chinese title
\def\titleEn   {From Gray to RGB: Optimization and Extension of CNN-based Interpretable Model} % English title
...
\def\keywordsZh{可解釋的人工智慧, 深度學習, 色彩感知, 性能提升}
\def\keywordsEn{Explainable Artificial Intelligence, Deep Learning, Color Perception, Performance Enhancement}
...
\def\letterAuthEl{pdfs/letter_authorization.pdf}
% 碩博士紙本論文延後公開/下架申請書。(如需延後公開者,才需要裝訂於論文內頁)
\def\letterPubReq{pdfs/letter_publication_request.pdf}
% 指導教授推薦書
\def\letterRecom {pdfs/letter_recommendation.pdf}
% 口試委員審定書
\def\letterVerif {pdfs/letter_verification.pdf}
% 學術倫理修課證明
\def\letterCareCert {pdfs/letter_caree_certification.pdf}
% 論文相似度比對報告電子回條
\def\letterThesisSim {pdfs/letter_thesis_simularity.pdf}
% 遠距口試申請
\def\letterRemoteExam {pdfs/letter_remote_exam.pdf}
