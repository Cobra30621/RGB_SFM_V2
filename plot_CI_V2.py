...
if arch['args']['in_channels'] == 1:
    ...
    print(model.RGB_convs[0][0].transform_weights())
...
if arch['args']['in_channels'] == 1:
    plot_map(
        FMs[0].reshape(int(FMs[0].shape[0] ** 0.5), int(FMs[0].shape[0] ** 0.5), *FMs[0].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_0')
    plot_map(
        FMs[1].reshape(int(FMs[1].shape[0] ** 0.5), int(FMs[1].shape[0] ** 0.5), *FMs[1].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_1')
    plot_map(
        FMs[2].reshape(int(FMs[2].shape[0] ** 0.5), int(FMs[2].shape[0] ** 0.5), *FMs[2].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_2')
    plot_map(
        FMs[3].reshape(int(FMs[3].shape[0] ** 0.5), int(FMs[3].shape[0] ** 0.5), *FMs[3].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_3')
else:
    plot_map(FMs['RGB_convs_0'].permute(0, 2, 3, 1).reshape(5, 6, *arch['args']['Conv2d_kernel'][0],
                                                            arch['args']['in_channels']).detach().numpy(),
             path=FMs_save_path + '/FMs_RGB_convs_0')
    plot_map(
        FMs['RGB_convs_1'].reshape(int(FMs['RGB_convs_1'].shape[0] ** 0.5), int(FMs['RGB_convs_1'].shape[0] ** 0.5),
                                   *FMs['RGB_convs_1'].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_RGB_convs_1')
    plot_map(
        FMs['RGB_convs_2'].reshape(int(FMs['RGB_convs_2'].shape[0] ** 0.5), int(FMs['RGB_convs_2'].shape[0] ** 0.5),
                                   *FMs['RGB_convs_2'].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_RGB_convs_2')

    plot_map(
        FMs['Gray_convs_0'].permute(0, 2, 3, 1).reshape(7, 10, *arch['args']['Conv2d_kernel'][0], 1).detach().numpy(),
        path=FMs_save_path + '/FMs_Gray_convs_0')
    plot_map(
        FMs['Gray_convs_1'].reshape(int(FMs['Gray_convs_1'].shape[0] ** 0.5), int(FMs['Gray_convs_1'].shape[0] ** 0.5),
                                    *FMs['Gray_convs_1'].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_Gray_convs_1')
    plot_map(
        FMs['Gray_convs_2'].reshape(int(FMs['Gray_convs_2'].shape[0] ** 0.5), int(FMs['Gray_convs_2'].shape[0] ** 0.5),
                                    *FMs['Gray_convs_2'].shape[1:]).detach().numpy(),
        path=FMs_save_path + '/FMs_Gray_convs_2')
print('FM saved')
...
if arch['args']['in_channels'] == 1:
    plot_map(
        ...
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_0')
    plot_map(
        CIs[1].reshape(int(CIs[1].shape[0] ** 0.5), int(CIs[1].shape[0] ** 0.5), *CIs[1].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_1')
    plot_map(
        CIs[2].reshape(int(CIs[2].shape[0] ** 0.5), int(CIs[2].shape[0] ** 0.5), *CIs[2].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_2')
    plot_map(
        CIs[3].reshape(int(CIs[3].shape[0] ** 0.5), int(CIs[3].shape[0] ** 0.5), *CIs[3].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_3')
    # 畫純灰階的
    plot_map(
        CIs[0].reshape(int(CIs[0].shape[0] ** 0.5), int(CIs[0].shape[0] ** 0.5), *CIs[0].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_0_gray', cmap='gray')
    plot_map(
        CIs[1].reshape(int(CIs[1].shape[0] ** 0.5), int(CIs[1].shape[0] ** 0.5), *CIs[1].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_1_gray', cmap='gray')
    plot_map(
        CIs[2].reshape(int(CIs[2].shape[0] ** 0.5), int(CIs[2].shape[0] ** 0.5), *CIs[2].shape[2:]).detach().numpy(),
        vmax=1, vmin=0, path=CIs_save_path + '/CIs_2_gray', cmap='gray')
    plot_map(
        CIs[3].reshape(int(CIs[3].shape[0] ** 0.5), int(CIs[3].shape[0] ** 0.5), *CIs[3].shape[2:]).detach().numpy(),
