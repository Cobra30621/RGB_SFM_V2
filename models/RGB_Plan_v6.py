...
class RGB_Conv2d(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        max_color = 1
        min_color = 0
        self.rgb_weight = torch.linspace(max_color, min_color, int((max_color - min_color) / (max_color / self.out_channels)), device = self.device)[:, None].repeat(1, 3)
        self.rgb_weight = nn.Parameter(self.rgb_weight)

        init.kaiming_uniform_(self.gray_weight, a=math.sqrt(5))
        self.gray_weight[0] = torch.zeros_like(self.gray_weight[0], device = self.device)
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        gray_input = Grayscale()(input)
        
        # RBF
        for k in range(rgb_result.shape[2]):
            for l in range(rgb_result.shape[3]):   
                window = input[:, :, k*stride:k*stride+self.kernel_size[0], l*stride:l*stride+self.kernel_size[1]]
                gray_window = gray_input[:, :, k*stride:k*stride+self.kernel_size[0], l*stride:l*stride+self.kernel_size[1]]

                dist = torch.zeros((batch_size, rgb_weight_expand.shape[0]), device = input.device)
                for in_channel in range(input.shape[1]):
                    dist += torch.cdist(window[:, in_channel].reshape(batch_size, -1), rgb_weight_expand[:, in_channel].reshape(rgb_weight_expand.shape[0], -1))
                dist += torch.cdist(gray_window[:, 0].reshape(batch_size, -1), self.gray_weight.reshape(self.gray_weight.shape[0], -1))
